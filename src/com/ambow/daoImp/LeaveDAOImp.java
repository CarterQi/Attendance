...
public class LeaveDAOImp implements LeaveDAO{
	...
	public ArrayList<LeaveInfo> queryname() throws SQLException{
		...
		String sql="select typeid,typename from Att_Attendancetype where typecategory='1'";
	...
	public ArrayList<NotesInfo> query(String name) throws SQLException{
		...
		String sql="select noteid,fillintime,cause,adminid,startdate,E.employeename employeename,typename,starttime,endtime,N.employeename operatorname from Att_notes left join Att_employees E "
				+ "on E.employeeid=Att_notes.employeeid left join Att_employees N on N.employeeid=Att_notes.operatorid left join Att_attendancetype on notetypeid=typeid where adminid=(select adminid from Att_User where adminname=?) and typecategory='1'";
		conn.prepareStatement(sql);
	...
	public void add(NotesInfo note) throws SQLException {
					...
					String sql="insert into Att_notes(employeeid,operatorid,notetypeid,"
							+ "fillintime,startdate,starttime,endtime,cause,adminid,directorsign,administrationsign,presidentsign)"
							+ "values((select employeeid from Att_employees where employeename=?),"
							+ "(select employeeid from Att_employees where employeename=?),"
							+ "(select typeid from Att_attendancetype where typename=?),?,?,?,?,?,(select adminid from Att_user where adminname=?),?,?,?)";
					DBUtil.execUpdate(sql, note.getEmployeename(),note.getOperatorname(),note.getTypename(),note.getFillintime(),
							note.getStarttime(),note.getStarttime(),note.getEndtime(),note.getCause(),note.getAdminname(),note.getDirectorsign(),
							note.getAdministrationsign(),note.getPersidentsign());
